...
/** The controller manages the underlying data model, performs the evaluation of jobs, and notifies
 * any listeners when its state changes. */
public class Controller {
	...
	/** The last accumulator to be generated; or {@code null} if no last accumulator exists or has
	...
	/** {@code true} if the generational distance indicator collector is included; {@code false}
	 * otherwise. */
	private boolean includeGenerationalDistance= true;

	/** {@code true} if the inverted generational distance indicator collector is included;
	...
	/** {@code true} if the additive &epsilon;-indicator collector is included; {@code false}
	 * otherwise. */
	private boolean includeAdditiveEpsilonIndicator= true;

	/** {@code true} if the contribution indicator collector is included; {@code false}
	 * otherwise. */
	...
	/** {@code true} if the adaptive multimethod variation collector is included; {@code false}
	 * otherwise. */
	private boolean includeAdaptiveMultimethodVariation= true;

	/** {@code true} if the adaptive time continuation collector is included; {@code false}
	 * otherwise. */
	...
	/** Toggles between showing individual trace lines when {@code true} and quantiles when
	 * {@code false}. */
	...
	/** Fires a {@code PARAM_CHANGED} controller event. */
	protected void fireParamChangedEvent() {
		fireEvent(new ControllerEvent(this,
			ControllerEvent.Type.PARAM_CHANGED));
	}

	/** Fires the specified controller event. All listeners will receive this event on the event
	 * dispatch thread.
	 *
	...
	/** Adds a new result to this controller. If the specified key already exists, the accumulator
	 * is appended to the existing results. A {@code MODEL_CHANGED} event is fired.
	 *
	 * @param key         the result key identifying the algorithm and problem associated with these
	 *                    results
	 ...
	 * @param accumulator the accumulator storing the results */
	public void add(String algorithm, String problem, String parameterization,
		Accumulator accumulator) {
		add(new ResultKey(algorithm, problem, parameterization), accumulator);
	}
	 ...
	 * @return an unmodifiable collection containing the results associated with the specified
	 *         key */
	...
	/** Returns the last accumulator to be generated; or {@code null} if no last accumulator exists
	 * or has been previously cleared.
	 *
	 * @return the last accumulator to be generated; or {@code null} if no last accumulator exists
	 *         or has been previously cleared */
	...
	/** Clears the last accumulator. Subsequent invocations of {@link #getLastAccumulator()} will
	 * return {@code null} until a new accumulator is generated. */
	...
	protected void updateProgress(int currentEvaluation, int currentSeed,
		...
		overallProgress= (int) (100 * (currentSeed > 0 ? currentSeed - 1 : 0) /
			(double) totalSeeds);

		fireProgressChangedEvent();
	}

	/** Creates and displays a dialog containing a statistical comparison of the selected
	 * results. */
	...
	public void run() {
		...
		final String param= frame.getParameterization().toString();

		// get borg parameter information
		final double sbxDI= frame.getsbxDistributionIndex();
		final double sbxRate= frame.getsbxRate();
		final double pmRate= frame.getpmRate();
		final double pmDI= frame.getpmDistributionIndex();
		final double deCrossoverRate= frame.getdeCrossoverRate();
		final double deStepSize= frame.getdeStepSize();
		final double umRate= frame.getumRate();
		final double spxEpsilon= frame.getspxEpsilon();
		final int spxParents= frame.getspxParents();
		final int spxOffspring= frame.getspxOffspring();
		final double pcxEta= frame.getpcxEta();
		final double pcxZeta= frame.getpcxZeta();
		final int pcxParents= frame.getpcxParents();
		final int pcxOffspring= frame.getpcxOffspring();
		final double undxZeta= frame.getundxZeta();
		final double undxEta= frame.getundxEta();
		final int undxParents= frame.getundxParents();
		final int undxOffspring= frame.getundxOffspring();
			...
			public void run() {
				try {
					...
					ProgressListener listener= new ProgressListener() {
						...
						public void progressUpdate(ProgressEvent event) {
							...
							if (event.isSeedFinished()) {
								...
								add(algorithmName, problemName, param,
					...
					executor= new Executor()
						...
						.withProperty("sbx.distributionIndex", sbxDI)
						.withProperty("pm.rate", pmRate)
						.withProperty("pm.distributionIndex", pmDI)
						.withProperty("de.crossoverRate", deCrossoverRate)
						.withProperty("pm.stepSize", deStepSize)
						.withProperty("um.rate", umRate)
						.withProperty("spx.epsilon", spxEpsilon)
						.withProperty("spx.parents", spxParents)
						.withProperty("spx.offspring", spxOffspring)
						.withProperty("pcx.eta", pcxEta)
						.withProperty("pcx.zeta", pcxZeta)
						.withProperty("pcx.parents", pcxParents)
						.withProperty("pcx.offspring", pcxOffspring)
						.withProperty("undx.zeta", undxZeta)
						.withProperty("undx.eta", undxEta)
						.withProperty("undx.parents", undxParents)
						.withProperty("undx.offspring", undxOffspring);
	...
	/** Returns {@code true} if this controller is currently processing an evaluation job;
	 * {@code false} otherwise.
	 *
	 * @return {@code true} if this controller is currently processing an evaluation job;
	 *         {@code false} otherwise */
	 ...
	 *
	 * @param showLastTrace {@code true} if the last run's trace is displayed; {@code false}
	 *                      otherwise */
	...
	/** Returns {@code true} if the hypervolume indicator collector is included; {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if the hypervolume indicator collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeHypervolume {@code true} if the hypervolume collector is included;
	 *                           {@code false} otherwise */
	public void setIncludeHypervolume(boolean includeHypervolume) {
		this.includeHypervolume= includeHypervolume;
	}

	/** Returns {@code true} if the generational distance indicator collector is included;
	 * {@code false} otherwise.
	 *
	 * @return {@code true} if the generational distance indicator collector is included;
	 *         {@code false} otherwise */
	 ...
	 *
	 * @param includeGenerationalDistance {@code true} if the generational distance indicator
	...
	/** Returns {@code true} if the inverted generational distance indicator collector is included;
	 * {@code false} otherwise.
	 *
	 * @return {@code true} if the inverted generational distance indicator collector is included;
	 *         {@code false} otherwise */
	 ...
	 *
	 * @param includeInvertedGenerationalDistance {@code true} if the inverted generational distance
	 *                                            indicator collector is included; {@code false}
	 *                                            otherwise */
	...
	/** Returns {@code true} if the spacing indicator collector is included; {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if the spacing indicator collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeSpacing {@code true} if the spacing indicator collector is included;
	 *                       {@code false} otherwise */
	public void setIncludeSpacing(boolean includeSpacing) {
		this.includeSpacing= includeSpacing;
	}

	/** Returns {@code true} if the additive &epsilon;-indicator collector is included;
	 * {@code false} otherwise.
	 *
	 * @return {@code true} if the additive &epsilon;-indicator collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeAdditiveEpsilonIndicator {@code true} if the additive &epsilon;-indicator
	 *                                        collector is included; {@code false} otherwise */
	...
	/** Returns {@code true} if the contribution indicator collector is included; {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if the contribution indicator collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeContribution {@code true} if the contribution indicator collector is included;
	 *                            {@code false} otherwise */
	 ...
	 *
	 * @param includeR1 {@code true} if the R1 indicator collector is included; {@code false}
	 *                  otherwise */
	 ...
	 *
	 * @param includeR2 {@code true} if the R2 indicator collector is included; {@code false}
	 *                  otherwise */
	 ...
	 *
	 * @param includeR3 {@code true} if the R3 indicator collector is included; {@code false}
	 *                  otherwise */
	public void setIncludeR3(boolean includeR3) {
		this.includeR3= includeR3;
	}

	/** Returns {@code true} if the &epsilon;-progress collector is included; {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if the &epsilon;-progress collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeEpsilonProgress {@code true} if the &epsilon;-progress collector is included;
	 *                               {@code false} otherwise */
	public void setIncludeEpsilonProgress(boolean includeEpsilonProgress) {
		this.includeEpsilonProgress= includeEpsilonProgress;
	}

	/** Returns {@code true} if the adaptive multimethod variation collector is included;
	 * {@code false} otherwise.
	 *
	 * @return {@code true} if the adaptive multimethod variation collector is included;
	 *         {@code false} otherwise */
	 ...
	 *
	 * @param includeAdaptiveMultimethodVariation {@code true} if the adaptive multimethod variation
	 *                                            collector is included; {@code false} otherwise */
	...
	/** Returns {@code true} if the adaptive time continuation collector is included; {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if the adaptive time continuation collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeAdaptiveTimeContinuation {@code true} if the adaptive time continuation
	 *                                        collector is included; {@code false} otherwise */
	 ...
	 *
	 * @param includeElapsedTime {@code true} if the elapsed time collector is included;
	 *                           {@code false} otherwise */
	public void setIncludeElapsedTime(boolean includeElapsedTime) {
		this.includeElapsedTime= includeElapsedTime;
	}

	/** Returns {@code true} if the approximation set collector is included; {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if the approximation set collector is included; {@code false}
	 *         otherwise */
	 ...
	 *
	 * @param includeApproximationSet {@code true} if the approximation set collector is included;
	 *                                {@code false} otherwise */
	 ...
	 *
	 * @param includePopulationSize {@code true} if the population size collector is included;
	 *                              {@code false} otherwise */
	public void setIncludePopulationSize(boolean includePopulationSize) {
		this.includePopulationSize= includePopulationSize;
	}

	/** Returns the run progress of the current job being evaluated. The run progress measures the
	 * number of evaluations completed thus far.
	 *
	...
	/** Returns the overall progress of the current job being evaluated. The overall progress
	 * measures the number of seeds completed thus far.
	 *
	 ...
	 *
	 * @param showIndividualTraces {@code true} to show individual traces; {@code false} to show
	 *                             quantiles */
	 ...
	 *                             quantiles
	 * @throws IOException */
	public void updateParameterization() throws IOException {
		frame.updateParameterizationPlot();
	}
